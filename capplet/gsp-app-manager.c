...
static void _gsp_app_manager_app_unref(GspApp *app, GspAppManager *manager);
static void _gsp_app_manager_app_removed(GspAppManager *manager, GspApp *app);

static GspXdgDir *_gsp_xdg_dir_new(const char *dir, int index) {
  GspXdgDir *xdgdir;
...
static void gsp_app_manager_class_init(GspAppManagerClass *class) {
  GObjectClass *gobject_class = G_OBJECT_CLASS(class);

  gobject_class->dispose = gsp_app_manager_dispose;
  gobject_class->finalize = gsp_app_manager_finalize;

  gsp_app_manager_signals[ADDED] = g_signal_new(
      "added", G_TYPE_FROM_CLASS(gobject_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GspAppManagerClass, added), NULL, NULL,
      g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, G_TYPE_OBJECT);

  gsp_app_manager_signals[REMOVED] = g_signal_new(
      "removed", G_TYPE_FROM_CLASS(gobject_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GspAppManagerClass, removed), NULL, NULL,
      g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, G_TYPE_OBJECT);
}

static void gsp_app_manager_init(GspAppManager *manager) {
...
static void gsp_app_manager_dispose(GObject *object) {
  ...
  g_slist_foreach(priv->apps, (GFunc)_gsp_app_manager_app_unref, manager);
...
static void gsp_app_manager_finalize(GObject *object) {
  ...
  g_slist_foreach(priv->dirs, (GFunc)_gsp_xdg_dir_free, NULL);
...
static int _gsp_app_manager_find_dir_with_basename(GspAppManager *manager,
                                                   const char *basename,
                                                   int minimum_index) {
  GSList *l;
  GspXdgDir *xdgdir;
...
static void _gsp_app_manager_handle_delete(GspAppManager *manager, GspApp *app,
  ...
  if (system_position == index) {
    ...
    new_system =
        _gsp_app_manager_find_dir_with_basename(manager, basename, index);
    if (new_system < 0) {
  ...
  if (position == index) {
    ...
    if (dir) {
      ...
      gsp_app_reload_at(app, path, (unsigned int)system_position);
      g_free(path);
    } else {
      _gsp_app_manager_app_removed(manager, app);
    }
...
static gboolean gsp_app_manager_xdg_dir_monitor(GFileMonitor *monitor,
  ...
  switch (flags) {
    ...
    case G_FILE_MONITOR_EVENT_DELETED:
      ...
      _gsp_app_manager_handle_delete(manager, old_app, basename, index);
      break;
    default:
      break;
  }
...
static void _gsp_app_manager_fill_from_dir(GspAppManager *manager,
                                           GspXdgDir *xdgdir) {
  GFile *file;
  GDir *dir;
  const char *name;
  ...
  if (xdgdir->monitor) {
    g_signal_connect(xdgdir->monitor, "changed",
                     G_CALLBACK(gsp_app_manager_xdg_dir_monitor), manager);
  }
...
static void _gsp_app_manager_app_unref(GspApp *app, GspAppManager *manager) {
  g_signal_handlers_disconnect_by_func(app, _gsp_app_manager_app_removed,
                                       manager);
  g_object_unref(app);
}
