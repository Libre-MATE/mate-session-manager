 ...
 *
 * [1]
 * http://git.fedorahosted.org/git/?p=desktop-effects.git;a=blob_plain;f=desktop-effects.c;hb=HEAD
 */
 ...
 * http://git.fedorahosted.org/git/?p=desktop-effects.git;a=blob_plain;f=desktop-effects.c;hb=HEAD
 * [2]
 * http://svn.mandriva.com/cgi-bin/viewvc.cgi/soft/drak3d/trunk/lib/Xconfig/glx.pm?view=markup
...
static inline void _print_error(const char *str) {
...
static int _parse_kcmdline(void) {
  ...
  if (!g_file_get_contents("/proc/cmdline", &contents, NULL, NULL)) return ret;

  regex = g_regex_new("mate.fallback=(\\S+)", 0, G_REGEX_MATCH_NOTEMPTY, NULL);
  if (!g_regex_match(regex, contents, G_REGEX_MATCH_NOTEMPTY, &match)) goto out;
  ...
  if (*arg != '0' && *arg != '1')
    fprintf(stderr,
            "mate-session-check-accelerated: Invalid value '%s' for "
            "mate.fallback passed in kernel command line.\n",
            arg);
  else
    ret = atoi(arg);
  g_free(word);
...
static int _parse_kcmdline(void) {
  ...
  if (kenv(KENV_GET, "mate.fallback", value, KENV_MVALLEN) == -1) return ret;

  if (*value != '0' && *value != '1')
    fprintf(stderr,
            "mate-session-is-accelerated: Invalid value '%s' for mate.fallback "
...
#else
static int _parse_kcmdline(void) { return CMDLINE_UNSET; }
#endif

static gboolean _has_composite(Display *display) {
  int dummy1, dummy2;

  return XCompositeQueryExtension(display, &dummy1, &dummy2);
}

static gboolean _is_comment(const char *line) {
...
static gboolean _is_gl_renderer_blacklisted(const char *renderer) {
  FILE *blacklist;
  char *line = NULL;
  size_t line_len = 0;
  gboolean ret = TRUE;
  ...
  while (getline(&line, &line_len, blacklist) != -1) {
    ...
    if (regcomp(&re, re_str, REG_EXTENDED | REG_ICASE | REG_NOSUB) != 0) {
      _print_error(
          "Cannot use this regular expression for hardware compatibility:");
      _print_error(re_str);
    } else {
...
out:
  ...
  if (blacklist != NULL) fclose(blacklist);

  return ret;
}

static char *_get_hardware_gl(Display *display) {
  int screen;
  Window root;
  ...
  int attrlist[] = {GLX_RGBA, GLX_RED_SIZE,  1, GLX_GREEN_SIZE,
  ...
  cwa.colormap = XCreateColormap(display, root, visual->visual, AllocNone);
  cwa.background_pixel = 0;
  cwa.border_pixel = 0;
  window = XCreateWindow(display, root, 0, 0, 1, 1, 0, visual->depth,
                         InputOutput, visual->visual,
                         CWColormap | CWBackPixel | CWBorderPixel, &cwa);

  if (!glXMakeCurrent(display, window, context)) goto out;
  ...
  if (renderer && strcasestr(renderer, "llvmpipe")) has_llvmpipe = TRUE;
  ...
  if (glGetError() != GL_NO_ERROR) max_texture_size = SIZE_ERROR;

  glGetIntegerv(GL_MAX_RENDERBUFFER_SIZE_EXT, &max_renderbuffer_size);
  if (glGetError() != GL_NO_ERROR) max_renderbuffer_size = SIZE_ERROR;

out:
  glXMakeCurrent(display, None, None);
  if (context) glXDestroyContext(display, context);
  if (window) XDestroyWindow(display, window);
  if (cwa.colormap) XFreeColormap(display, cwa.colormap);

  return renderer;
}

static gboolean _has_extension(const char *extension_list,
...
static gboolean _has_texture_from_pixmap(Display *display) {
  int screen;
  ...
  if (!_has_extension(server_extensions, "GLX_EXT_texture_from_pixmap"))
    goto out;

  client_extensions = glXGetClientString(display, GLX_EXTENSIONS);
  if (!_has_extension(client_extensions, "GLX_EXT_texture_from_pixmap"))
    goto out;
...
static void _set_max_screen_size_property(Display *display, int screen,
                                          int size) {
  Atom max_screen_size_atom;

  max_screen_size_atom = XInternAtom(display, "_GNOME_MAX_SCREEN_SIZE", False);
  ...
  XChangeProperty(display, RootWindow(display, screen), max_screen_size_atom,
                  XA_CARDINAL, 32, PropModeReplace, (unsigned char *)&size, 1);

  XSync(display, False);
}

static gboolean _is_max_texture_size_big_enough(Display *display) {
  int screen, size;
...
static const GOptionEntry entries[] = {
    {"print-renderer", 'p', 0, G_OPTION_ARG_NONE, &print_renderer,
     "Print GL renderer name", NULL},
...
int main(int argc, char **argv) {
  ...
  if (print_renderer) g_print("%s", renderer);
