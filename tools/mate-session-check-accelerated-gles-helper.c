...
static char *get_gles_renderer(void) {
  ...
  win = XCreateWindow(display, DefaultRootWindow(display), 0, 0, /* x, y */
                      1, 1, /* width, height */
                      0,    /* border_width */
...
out:
  ...
  if (egl_inited) eglTerminate(egl_dpy);
  if (win != None) XDestroyWindow(display, win);
