...
static gboolean on_property_notify_timeout(gpointer data) {
  gtk_main_quit();
  return FALSE;
}

static GdkFilterReturn property_notify_filter(GdkXEvent *xevent,
...
static gboolean wait_for_property_notify(void) {
  GdkDisplay *display;
  GdkScreen *screen;
  GdkWindow *root;
  Window rootwin;
...
static char *get_gtk_gles_renderer(void) {
  GtkWidget *win;
...
static gboolean is_discrete_gpu_check(void) {
  const char *dri_prime;

  dri_prime = g_getenv("DRI_PRIME");
  if (!dri_prime) return FALSE;
  if (*dri_prime != '1') return FALSE;
  return TRUE;
}
...
int main(int argc, char **argv) {
  ...
  int estatus;
  char *gl_helper_argv[] = {LIBEXECDIR
                            "/mate-session-check-accelerated-gl-helper",
                            "--print-renderer", NULL};
#ifdef HAVE_GLESV2
  char *gles_helper_argv[] = {LIBEXECDIR
   ...
   */
  if (is_discrete_gpu_check() ||
      g_strcmp0(g_getenv("XDG_SESSION_TYPE"), "x11") != 0) {
  ...
  is_accelerated_atom = gdk_x11_get_xatom_by_name_for_display(
      display, "_GNOME_SESSION_ACCELERATED");
  is_software_rendering_atom = gdk_x11_get_xatom_by_name_for_display(
      display, "_GNOME_IS_SOFTWARE_RENDERING");
  renderer_atom =
      gdk_x11_get_xatom_by_name_for_display(display, "_GNOME_SESSION_RENDERER");
  ...
  read:
    ...
    XGetWindowProperty(GDK_DISPLAY_XDISPLAY(display), rootwin,
                       is_accelerated_atom, 0, G_MAXLONG, False, XA_CARDINAL,
    ...
    if (type == XA_CARDINAL) {
      ...
      if (*is_accelerated_ptr == ACCEL_CHECK_RUNNING) {
        ...
        if (wait_for_property_notify()) goto read;
        /* else fall through and do the check ourselves */

      } else {
        gdk_x11_display_error_trap_push(display);
        XGetWindowProperty(GDK_DISPLAY_XDISPLAY(display), rootwin,
                           renderer_atom, 0, G_MAXLONG, False, XA_STRING, &type,
  ...
  XChangeProperty(GDK_DISPLAY_XDISPLAY(display), rootwin, is_accelerated_atom,
                  XA_CARDINAL, 32, PropModeReplace, (guchar *)&is_accelerated,
                  1);

  gdk_display_sync(display);

  /* First, try the GL helper */
  if (g_spawn_sync(NULL, (char **)gl_helper_argv, NULL, 0, NULL, NULL,
                   &gl_renderer_string, NULL, &estatus, &gl_error)) {
    ...
    g_printerr(
        "mate-session-check-accelerated: GL Helper exited with code %d\n",
        estatus);
  }

#ifdef HAVE_GLESV2
  /* Then, try the GLES helper */
  if (g_spawn_sync(NULL, (char **)gles_helper_argv, NULL, 0, NULL, NULL,
                   &gles_renderer_string, NULL, &estatus, &gles_error)) {
    is_accelerated = (WEXITSTATUS(estatus) == HELPER_ACCEL);
    gles_software_rendering =
        (WEXITSTATUS(estatus) == HELPER_SOFTWARE_RENDERING);
    if (is_accelerated) {
    ...
    g_printerr(
        "mate-session-check-accelerated: GLES Helper exited with code %d\n",
        estatus);
  }
#endif
  ...
  if (gl_software_rendering || gles_software_rendering) {
    ...
    if (gl_software_rendering) renderer_string = gl_renderer_string;
#ifdef HAVE_GLESV2
  ...
  if (gles_error != NULL) {
    g_printerr(
        "mate-session-check-accelerated: Failed to run GLES helper: %s\n",
        gles_error->message);
...
finish:
  ...
  if (is_software_rendering) {
    XChangeProperty(GDK_DISPLAY_XDISPLAY(display), rootwin,
                    is_software_rendering_atom, XA_CARDINAL, 32,
                    PropModeReplace, (guchar *)&is_software_rendering, 1);
  }

  if (renderer_string != NULL) {
    XChangeProperty(GDK_DISPLAY_XDISPLAY(display), rootwin, renderer_atom,
                    XA_STRING, 8, PropModeReplace, (guchar *)renderer_string,
                    strlen(renderer_string));
