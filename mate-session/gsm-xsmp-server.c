...
static gboolean ice_protocol_timeout(IceConn ice_conn) {
...
static gboolean auth_iochannel_watch(GIOChannel *source, GIOCondition condition,
                                     IceConn ice_conn) {
  GsmIceConnectionWatch *data;
  gboolean keep_going;
  ...
  switch (IceProcessMessages(ice_conn, NULL, NULL)) {
    ...
    case IceProcessMessagesIOError:
      g_debug(
          "GsmXsmpServer: IceProcessMessages returned "
...
/* IceAcceptConnection returns a new ICE connection that is in a "pending"
 * state, this is because authentification may be necessary. So we've to
 * authenticate it, before accept_xsmp_connection() is called. Then each
 * GsmXSMPClient will have its own IceConn watcher
 */
static void auth_ice_connection(IceConn ice_conn) {
  GIOChannel *channel;
  GsmIceConnectionWatch *data;
  int fd;
  ...
  data->protocol_timeout =
      g_timeout_add_seconds(5, (GSourceFunc)ice_protocol_timeout, ice_conn);
  data->watch_id = g_io_add_watch(channel, G_IO_IN | G_IO_ERR,
                                  (GIOFunc)auth_iochannel_watch, ice_conn);
...
void gsm_xsmp_server_start(GsmXsmpServer *server) {
  ...
  for (i = 0; i < server->num_local_xsmp_sockets; i++) {
    ...
    channel = g_io_channel_unix_new(
        IceGetListenConnectionNumber(server->xsmp_sockets[i]));
    g_io_add_watch_full(
        channel, G_PRIORITY_DEFAULT, G_IO_IN | G_IO_HUP | G_IO_ERR,
        (GIOFunc)accept_ice_connection, data, (GDestroyNotify)g_free);
    g_io_channel_unref(channel);
  }
}

static void gsm_xsmp_server_set_client_store(GsmXsmpServer *xsmp_server,
                                             GsmStore *store) {
...
static void gsm_xsmp_server_set_property(GObject *object, guint prop_id,
                                         const GValue *value,
                                         GParamSpec *pspec) {
  GsmXsmpServer *self;
...
static void gsm_xsmp_server_get_property(GObject *object, guint prop_id,
 ...
 */
static Status accept_xsmp_connection(SmsConn sms_conn, GsmXsmpServer *server,
  ...
  if (server->xsmp_sockets == NULL) {
    ...
    *failure_reason_ret =
        strdup(_("Refusing new client connection because the session is "
  ...
  gsm_store_add(server->client_store, gsm_client_peek_id(client),
                G_OBJECT(client));
  /* the store will own the ref */
  g_object_unref(client);

  gsm_xsmp_client_connect(GSM_XSMP_CLIENT(client), sms_conn, mask_ret,
                          callbacks_ret);

  return TRUE;
}

static void ice_error_handler(IceConn conn, Bool swap,
                              int offending_minor_opcode,
                              unsigned long offending_sequence, int error_class,
                              int severity, IcePointer values) {
  g_debug("GsmXsmpServer: ice_error_handler (%p, %s, %d, %lx, %d, %d)", conn,
          swap ? "TRUE" : "FALSE", offending_minor_opcode, offending_sequence,
          error_class, severity);
...
static void sms_error_handler(SmsConn conn, Bool swap,
                              int offending_minor_opcode,
                              unsigned long offending_sequence_num,
                              int error_class, int severity,
                              IcePointer values) {
  g_debug("GsmXsmpServer: sms_error_handler (%p, %s, %d, %lx, %d, %d)", conn,
          swap ? "TRUE" : "FALSE", offending_minor_opcode,
...
static IceAuthFileEntry *auth_entry_new(const char *protocol,
...
static gboolean update_iceauthority(GsmXsmpServer *server, gboolean adding) {
  char *filename;
  ...
  if (adding) {
    for (i = 0; i < server->num_local_xsmp_sockets; i++) {
      entries =
          g_slist_append(entries, auth_entry_new("ICE", our_network_ids[i]));
      entries =
          g_slist_prepend(entries, auth_entry_new("XSMP", our_network_ids[i]));
    }
  }
...
static void setup_listener(GsmXsmpServer *server) {
  char error[256];
  mode_t saved_umask;
   ...
   */
  res =
      SmsInitialize(PACKAGE, VERSION, (SmsNewClientProc)accept_xsmp_connection,
                    server, NULL, sizeof(error), error);
  if (!res) {
  ...
  res = IceListenForConnections(&server->num_xsmp_sockets,
                                &server->xsmp_sockets, sizeof(error), error);
  if (!res) {
    gsm_util_init_error(TRUE, _("Could not create ICE listening socket: %s"),
                        error);
  }
   ...
   */
  for (i = server->num_local_xsmp_sockets = 0; i < server->num_xsmp_sockets;
       i++) {
  ...
  if (server->num_local_xsmp_sockets == 0) {
    gsm_util_init_error(
        TRUE, "IceListenForConnections did not return a local listener!");
  }
  ...
  if (server->num_local_xsmp_sockets != server->num_xsmp_sockets) {
     ...
     * calls above.
     */
    network_id_list = IceComposeNetworkIdList(
  ...
  if (!update_iceauthority(server, TRUE)) {
    ...
    gsm_util_init_error(TRUE, "Could not update ICEauthority file %s",
                        IceAuthFileName());
  }
...
static GObject *gsm_xsmp_server_constructor(
    GType type, guint n_construct_properties,
    GObjectConstructParam *construct_properties) {
  GsmXsmpServer *xsmp_server;

  xsmp_server = GSM_XSMP_SERVER(
      G_OBJECT_CLASS(gsm_xsmp_server_parent_class)
...
static void gsm_xsmp_server_finalize(GObject *object) {
  ...
  IceFreeListenObjs(xsmp_server->num_xsmp_sockets, xsmp_server->xsmp_sockets);
...
GsmXsmpServer *gsm_xsmp_server_new(GsmStore *client_store) {
  if (xsmp_server_object != NULL) {
    ...
    xsmp_server_object =
        g_object_new(GSM_TYPE_XSMP_SERVER, "client-store", client_store, NULL);
