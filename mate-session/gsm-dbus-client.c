...
#define SM_DBUS_CLIENT_PRIVATE_INTERFACE \
...
static gboolean setup_connection(GsmDBusClient *client) {
  DBusError error;
...
static void raise_error(DBusConnection *connection, DBusMessage *in_reply_to,
...
static void handle_end_session_response(GsmDBusClient *client,
                                        DBusMessage *message) {
  const char *sender;
  DBusMessage *reply;
  DBusError error;
  ...
  dbus_error_init(&error);
  if (!dbus_message_get_args(message, &error, DBUS_TYPE_BOOLEAN, &is_ok,
                             DBUS_TYPE_STRING, &reason, DBUS_TYPE_INVALID)) {
    if (dbus_error_is_set(&error)) {
      g_warning("Invalid method call: %s", error.message);
      dbus_error_free(&error);
    }
    raise_error(client->connection, message, DBUS_ERROR_FAILED,
                "There is a syntax error in the invocation of the method "
                "EndSessionResponse");
    return;
  }

  g_debug("GsmDBusClient: got EndSessionResponse is-ok:%d reason=%s", is_ok,
          reason);

  /* make sure it is from our client */
  sender = dbus_message_get_sender(message);
  if (sender == NULL || IS_STRING_EMPTY(client->bus_name) ||
  ...
  gsm_client_end_session_response(GSM_CLIENT(client), is_ok, FALSE, FALSE,
                                  reason);
...
static DBusHandlerResult client_dbus_filter_function(DBusConnection *connection,
                                                     DBusMessage *message,
                                                     void *user_data) {
  ...
  if (dbus_message_is_method_call(message, SM_DBUS_CLIENT_PRIVATE_INTERFACE,
                                  "EndSessionResponse")) {
    g_assert(gsm_client_peek_id(GSM_CLIENT(client)) != NULL);

    if (path != NULL &&
        strcmp(path, gsm_client_peek_id(GSM_CLIENT(client))) != 0) {
...
static GObject *gsm_dbus_client_constructor(
    GType type, guint n_construct_properties,
    GObjectConstructParam *construct_properties) {
  GsmDBusClient *client;

  client = GSM_DBUS_CLIENT(
      G_OBJECT_CLASS(gsm_dbus_client_parent_class)
  ...
  dbus_connection_add_filter(client->connection, client_dbus_filter_function,
                             client, NULL);

  return G_OBJECT(client);
}

static void gsm_dbus_client_init(GsmDBusClient *client) {}

/* adapted from PolicyKit */
static gboolean get_caller_info(GsmDBusClient *client, const char *sender,
                                uid_t *calling_uid, pid_t *calling_pid) {
  gboolean res;
  GError *error;
...
static void gsm_dbus_client_set_bus_name(GsmDBusClient *client,
                                         const char *bus_name) {
  uid_t uid;
  pid_t pid;
...
static void gsm_dbus_client_set_property(GObject *object, guint prop_id,
                                         const GValue *value,
                                         GParamSpec *pspec) {
  GsmDBusClient *self;
...
static void gsm_dbus_client_get_property(GObject *object, guint prop_id,
...
static void gsm_dbus_client_finalize(GObject *object) {
...
static GKeyFile *dbus_client_save(GsmClient *client, GError **error) {
...
static gboolean dbus_client_stop(GsmClient *client, GError **error) {
  ...
  message = dbus_message_new_signal(gsm_client_peek_id(client),
                                    SM_DBUS_CLIENT_PRIVATE_INTERFACE, "Stop");
  if (message == NULL) {
    goto out;
  }
...
static char *dbus_client_get_app_name(GsmClient *client) {
  /* Always use app-id instead */
  return NULL;
}

static GsmClientRestartStyle dbus_client_get_restart_style_hint(
    GsmClient *client) {
  return (GSM_DBUS_CLIENT(client)->restart_style_hint);
}

static guint dbus_client_get_unix_process_id(GsmClient *client) {
  return (GSM_DBUS_CLIENT(client)->caller_pid);
}

static gboolean dbus_client_query_end_session(GsmClient *client, guint flags,
                                              GError **error) {
  GsmDBusClient *dbus_client = (GsmDBusClient *)client;
  DBusMessage *message;
  ...
  g_debug("GsmDBusClient: sending QueryEndSession signal to %s",
...
static gboolean dbus_client_end_session(GsmClient *client, guint flags,
                                        GError **error) {
  GsmDBusClient *dbus_client = (GsmDBusClient *)client;
  DBusMessage *message;
  DBusMessageIter iter;
  ...
  /* unicast the signal to only the registered bus name */
  message =
      dbus_message_new_signal(gsm_client_peek_id(client),
                              SM_DBUS_CLIENT_PRIVATE_INTERFACE, "EndSession");
  if (message == NULL) {
    g_set_error(error, GSM_CLIENT_ERROR, GSM_CLIENT_ERROR_NOT_REGISTERED,
...
static gboolean dbus_client_cancel_end_session(GsmClient *client,
                                               GError **error) {
  GsmDBusClient *dbus_client = (GsmDBusClient *)client;
  DBusMessage *message;
  gboolean ret = FALSE;
...
static void gsm_dbus_client_dispose(GObject *object) {
  GsmDBusClient *client;
  ...
  dbus_connection_remove_filter(client->connection, client_dbus_filter_function,
                                client);

  G_OBJECT_CLASS(gsm_dbus_client_parent_class)->dispose(object);
}

static void gsm_dbus_client_class_init(GsmDBusClientClass *klass) {
  ...
  client_class->impl_get_restart_style_hint =
      dbus_client_get_restart_style_hint;
...
GsmClient *gsm_dbus_client_new(const char *startup_id, const char *bus_name) {
  GsmDBusClient *client;

  client = g_object_new(GSM_TYPE_DBUS_CLIENT, "startup-id", startup_id,
                        "bus-name", bus_name, NULL);
