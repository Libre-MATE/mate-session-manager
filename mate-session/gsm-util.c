...
gchar **gsm_get_screen_locker_command(void) {
  ...
  for (i = 0; screen_locker_command[i] != NULL && screen_locker_argv == NULL;
       i++) {
    gchar **argv;
    char *path;
    ...
    if (path) screen_locker_argv = g_strdupv(argv);
...
char *gsm_util_find_desktop_file_for_app_name(const char *name,
  ...
  g_key_file_load_from_dirs(key_file, desktop_file, (const char **)app_dirs,
                            &app_path, G_KEY_FILE_NONE, NULL);
  ...
  if (app_path == NULL && autostart_dirs != NULL) {
    g_key_file_load_from_dirs(key_file, desktop_file,
                              (const char **)autostart_dirs, &app_path,
...
static gboolean ensure_dir_exists(const char *dir) {
  if (g_file_test(dir, G_FILE_TEST_IS_DIR)) return TRUE;

  if (g_mkdir_with_parents(dir, 0755) == 0) return TRUE;

  if (errno == EEXIST) return g_file_test(dir, G_FILE_TEST_IS_DIR);
...
gchar *gsm_util_get_empty_tmp_session_dir(void) {
  ...
  tmp = g_build_filename(g_get_user_config_dir(), "mate-session",
                         "saved-session.new", NULL);

  exists = ensure_dir_exists(tmp);

  if (G_UNLIKELY(!exists)) {
    g_warning(
        "GsmSessionSave: could not create directory for saved session: %s",
        tmp);
    g_free(tmp);
    return NULL;
  } else {
...
const gchar *gsm_util_get_saved_session_dir(void) {
  if (_saved_session_dir == NULL) {
    gboolean exists;

    _saved_session_dir = g_build_filename(
        g_get_user_config_dir(), "mate-session", "saved-session", NULL);
...
char **gsm_util_get_autostart_dirs() {
  GPtrArray *dirs;
  ...
  for (i = 0; system_data_dirs[i]; i++) {
    g_ptr_array_add(
        dirs, g_build_filename(system_data_dirs[i], "mate", "autostart", NULL));
  }
...
char **gsm_util_get_desktop_dirs() {
  char **apps;
  char **autostart;
  char **result;
  int size;
...
char *gsm_util_generate_startup_id(void) {
  ...
  return g_strdup_printf("10%.04x%.04x%.10lu%.3u%.10lu%.4d", rand1, rand2,
                         (unsigned long)tv.tv_sec, (unsigned)tv.tv_usec,
                         (unsigned long)pid, sequence);
}

static gboolean gsm_util_update_activation_environment(const char *variable,
                                                       const char *value,
                                                       GError **error) {
  ...
  reply = g_dbus_connection_call_sync(
      ...
      G_DBUS_CALL_FLAGS_NONE, -1, NULL, &bus_error);
...
gboolean gsm_util_export_activation_environment(GError **error) {
  ...
  name_regex =
      g_regex_new("^[a-zA-Z_][a-zA-Z0-9_]*$", G_REGEX_OPTIMIZE, 0, error);
  ...
  value_regex =
      g_regex_new("^([[:blank:]]|[^[:cntrl:]])*$", G_REGEX_OPTIMIZE, 0, error);
  ...
  for (entry_names = g_listenv(); entry_names[i] != NULL; i++) {
    ...
    if (!g_utf8_validate(entry_name, -1, NULL)) continue;

    if (!g_regex_match(name_regex, entry_name, 0, NULL)) continue;

    if (!g_utf8_validate(entry_value, -1, NULL)) continue;

    if (!g_regex_match(value_regex, entry_value, 0, NULL)) continue;
...
gboolean gsm_util_export_user_environment(GError **error) {
  ...
  regex = g_regex_new("^[a-zA-Z_][a-zA-Z0-9_]*=([[:blank:]]|[^[:cntrl:]])*$",
  ...
  for (entries = g_get_environ(); entries[i] != NULL; i++) {
    ...
    if (!g_utf8_validate(entry, -1, NULL)) continue;

    if (!g_regex_match(regex, entry, 0, NULL)) continue;
  ...
  reply = g_dbus_connection_call_sync(
      ...
      g_variant_new("(@as)", g_variant_builder_end(&builder)), NULL,
      G_DBUS_CALL_FLAGS_NONE, -1, NULL, &bus_error);
...
static gboolean gsm_util_update_user_environment(const char *variable,
                                                 const char *value,
                                                 GError **error) {
  GDBusConnection *connection;
  gboolean environment_updated;
...
void gsm_util_setenv(const char *variable, const char *value) {
  ...
  if (!gsm_util_update_activation_environment(variable, value, &error)) {
    g_warning(
        "Could not make bus activated clients aware of %s=%s environment "
        "variable: %s",
  ...
  if (!gsm_util_update_user_environment(variable, value, &error)) {
    g_debug("Could not make systemd aware of %s=%s environment variable: %s",
            variable, value, error->message);
    g_clear_error(&error);
  }
#endif
