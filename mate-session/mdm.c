...
static MdmProtocolData mdm_protocol_data = {0, NULL, MDM_LOGOUT_ACTION_NONE,
...
#define MDM_MIT_MAGIC_COOKIE_LEN 16
  ...
  if (data->auth_cookie) {
    ...
    msg =
        g_strdup_printf(MDM_PROTOCOL_MSG_AUTHENTICATE " %s", data->auth_cookie);
...
static void mdm_parse_query_response(MdmProtocolData* data,
                                     const char* response) {
  char** actions;
  int i;
...
void mdm_new_login(void) {
  ...
  response =
      mdm_send_protocol_msg(&mdm_protocol_data, MDM_PROTOCOL_MSG_FLEXI_XSERVER);
