...
enum { STATUS_CHANGED, STATUS_TEXT_CHANGED, LAST_SIGNAL };
...
GType gsm_presence_error_get_type(void) {
  ...
  if (etype == 0) {
    static const GEnumValue values[] = {
        ENUM_ENTRY(GSM_PRESENCE_ERROR_GENERAL, "GeneralError"), {0, 0, 0}};
...
static void set_session_idle(GsmPresence *presence, gboolean is_idle) {
...
static gboolean on_idle_timeout(GSIdleMonitor *monitor, guint id,
                                gboolean condition, GsmPresence *presence) {
  gboolean handled;
...
static void reset_idle_watch(GsmPresence *presence) {
  GsmPresencePrivate *priv;
  ...
  if (priv->idle_watch_id > 0) {
    ...
    gs_idle_monitor_remove_watch(priv->idle_monitor, priv->idle_watch_id);
    priv->idle_watch_id = 0;
  }

  if (!priv->screensaver_active && priv->idle_enabled &&
      priv->idle_timeout > 0) {
    g_debug("GsmPresence: adding idle watch");

    priv->idle_watch_id = gs_idle_monitor_add_watch(
        priv->idle_monitor, priv->idle_timeout,
        (GSIdleMonitorWatchFunc)on_idle_timeout, presence);
  }
}

static void on_screensaver_active_changed(DBusGProxy *proxy, gboolean is_active,
...
static void on_screensaver_proxy_destroy(GObject *proxy,
                                         GsmPresence *presence) {
  GsmPresencePrivate *priv;
...
static void on_bus_name_owner_changed(DBusGProxy *bus_proxy,
  ...
  if (service_name == NULL || strcmp(service_name, GS_NAME) != 0) {
    /* ignore */
    return;
  }
  ...
  if (strlen(new_service_name) == 0 && strlen(old_service_name) > 0) {
    ...
    /* service added */
    error = NULL;
    priv->screensaver_proxy = dbus_g_proxy_new_for_name_owner(
        priv->bus_connection, GS_NAME, GS_PATH, GS_INTERFACE, &error);
    if (priv->screensaver_proxy != NULL) {
      g_signal_connect(priv->screensaver_proxy, "destroy",
                       G_CALLBACK(on_screensaver_proxy_destroy), presence);
      dbus_g_proxy_add_signal(priv->screensaver_proxy, "ActiveChanged",
                              G_TYPE_BOOLEAN, G_TYPE_INVALID);
      dbus_g_proxy_connect_signal(priv->screensaver_proxy, "ActiveChanged",
...
static gboolean register_presence(GsmPresence *presence) {
  GError *error;
  GsmPresencePrivate *priv;
  ...
  dbus_g_connection_register_g_object(
      priv->bus_connection, GSM_PRESENCE_DBUS_PATH, G_OBJECT(presence));

  return TRUE;
}

static GObject *gsm_presence_constructor(
  ...
  presence = GSM_PRESENCE(
      G_OBJECT_CLASS(gsm_presence_parent_class)
  ...
  priv->bus_proxy =
      dbus_g_proxy_new_for_name(priv->bus_connection, DBUS_SERVICE_DBUS,
  ...
  if (priv->bus_proxy != NULL) {
    ...
    dbus_g_proxy_connect_signal(priv->bus_proxy, "NameOwnerChanged",
                                G_CALLBACK(on_bus_name_owner_changed), presence,
                                NULL);
  }
...
static void gsm_presence_init(GsmPresence *presence) {
  GsmPresencePrivate *priv;
...
gboolean gsm_presence_set_status_text(GsmPresence *presence,
  ...
  if (status_text != NULL && strlen(status_text) > MAX_STATUS_TEXT) {
    g_set_error(error, GSM_PRESENCE_ERROR, GSM_PRESENCE_ERROR_GENERAL,
...
void gsm_presence_set_idle_timeout(GsmPresence *presence, guint timeout) {
  GsmPresencePrivate *priv;
...
static void gsm_presence_set_property(GObject *object, guint prop_id,
...
static void gsm_presence_get_property(GObject *object, guint prop_id,
...
static void gsm_presence_finalize(GObject *object) {
...
static void gsm_presence_class_init(GsmPresenceClass *klass) {
  ...
  signals[STATUS_CHANGED] = g_signal_new(
      "status-changed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GsmPresenceClass, status_changed), NULL, NULL,
      g_cclosure_marshal_VOID__UINT, G_TYPE_NONE, 1, G_TYPE_UINT);
  signals[STATUS_TEXT_CHANGED] = g_signal_new(
      "status-text-changed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GsmPresenceClass, status_text_changed), NULL, NULL,
      g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);

  g_object_class_install_property(
      object_class, PROP_STATUS,
      g_param_spec_uint("status", "status", "status", 0, G_MAXINT, 0,
                        G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
  g_object_class_install_property(
      object_class, PROP_STATUS_TEXT,
      g_param_spec_string("status-text", "status text", "status text", "",
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
  g_object_class_install_property(
      object_class, PROP_IDLE_ENABLED,
      g_param_spec_boolean("idle-enabled", NULL, NULL, FALSE,
                           G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
  g_object_class_install_property(
      object_class, PROP_IDLE_TIMEOUT,
      g_param_spec_uint("idle-timeout", "idle timeout", "idle timeout", 0,
                        G_MAXINT, 300000,
                        G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  dbus_g_object_type_install_info(GSM_TYPE_PRESENCE,
                                  &dbus_glib_gsm_presence_object_info);
  dbus_g_error_domain_register(GSM_PRESENCE_ERROR, NULL,
                               GSM_PRESENCE_TYPE_ERROR);
}

GsmPresence *gsm_presence_new(void) {
  GsmPresence *presence;

  presence = g_object_new(GSM_TYPE_PRESENCE, NULL);
