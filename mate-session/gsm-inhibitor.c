...
static guint32 get_next_inhibitor_serial(void) {
  guint32 serial;
...
static gboolean register_inhibitor(GsmInhibitor *inhibitor) {
  GError *error;
  ...
  dbus_g_connection_register_g_object(inhibitor->connection, inhibitor->id,
                                      G_OBJECT(inhibitor));

  return TRUE;
}

static GObject *gsm_inhibitor_constructor(
    GType type, guint n_construct_properties,
    GObjectConstructParam *construct_properties) {
  GsmInhibitor *inhibitor;
  gboolean res;

  inhibitor = GSM_INHIBITOR(
      G_OBJECT_CLASS(gsm_inhibitor_parent_class)
          ->constructor(type, n_construct_properties, construct_properties));

  g_free(inhibitor->id);
  inhibitor->id = g_strdup_printf("/org/gnome/SessionManager/Inhibitor%u",
                                  get_next_inhibitor_serial());
...
static void gsm_inhibitor_init(GsmInhibitor *inhibitor) {}

static void gsm_inhibitor_set_bus_name(GsmInhibitor *inhibitor,
                                       const char *bus_name) {
...
static void gsm_inhibitor_set_app_id(GsmInhibitor *inhibitor,
                                     const char *app_id) {
  g_return_if_fail(GSM_IS_INHIBITOR(inhibitor));
...
static void gsm_inhibitor_set_client_id(GsmInhibitor *inhibitor,
...
static void gsm_inhibitor_set_reason(GsmInhibitor *inhibitor,
                                     const char *reason) {
  g_return_if_fail(GSM_IS_INHIBITOR(inhibitor));
...
static void gsm_inhibitor_set_cookie(GsmInhibitor *inhibitor, guint cookie) {
  g_return_if_fail(GSM_IS_INHIBITOR(inhibitor));
...
static void gsm_inhibitor_set_flags(GsmInhibitor *inhibitor, guint flags) {
  g_return_if_fail(GSM_IS_INHIBITOR(inhibitor));
...
static void gsm_inhibitor_set_toplevel_xid(GsmInhibitor *inhibitor, guint xid) {
  g_return_if_fail(GSM_IS_INHIBITOR(inhibitor));
...
gboolean gsm_inhibitor_get_app_id(GsmInhibitor *inhibitor, char **id,
                                  GError **error) {
...
gboolean gsm_inhibitor_get_client_id(GsmInhibitor *inhibitor, char **id,
                                     GError **error) {
  g_return_val_if_fail(GSM_IS_INHIBITOR(inhibitor), FALSE);

  /* object paths are not allowed to be NULL or blank */
  if (IS_STRING_EMPTY(inhibitor->client_id)) {
    g_set_error(error, GSM_INHIBITOR_ERROR, GSM_INHIBITOR_ERROR_NOT_SET,
                "Value is not set");
    return FALSE;
  }
...
gboolean gsm_inhibitor_get_reason(GsmInhibitor *inhibitor, char **reason,
                                  GError **error) {
  g_return_val_if_fail(GSM_IS_INHIBITOR(inhibitor), FALSE);
...
gboolean gsm_inhibitor_get_flags(GsmInhibitor *inhibitor, guint *flags,
                                 GError **error) {
  g_return_val_if_fail(GSM_IS_INHIBITOR(inhibitor), FALSE);
...
gboolean gsm_inhibitor_get_toplevel_xid(GsmInhibitor *inhibitor, guint *xid,
                                        GError **error) {
  g_return_val_if_fail(GSM_IS_INHIBITOR(inhibitor), FALSE);
...
static void gsm_inhibitor_set_property(GObject *object, guint prop_id,
...
static void gsm_inhibitor_get_property(GObject *object, guint prop_id,
...
static void gsm_inhibitor_finalize(GObject *object) {
...
static void gsm_inhibitor_class_init(GsmInhibitorClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("bus-name", "bus-name", "bus-name", "",
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("app-id", "app-id", "app-id", "",
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
  g_object_class_install_property(
      object_class, PROP_CLIENT_ID,
      g_param_spec_string("client-id", "client-id", "client-id", "",
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
  g_object_class_install_property(
      object_class, PROP_REASON,
      g_param_spec_string("reason", "reason", "reason", "",
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
  g_object_class_install_property(
      object_class, PROP_FLAGS,
      g_param_spec_uint("flags", "flags", "flags", 0, G_MAXINT, 0,
                        G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
  g_object_class_install_property(
      object_class, PROP_TOPLEVEL_XID,
      g_param_spec_uint("toplevel-xid", "toplevel-xid", "toplevel-xid", 0,
                        G_MAXINT, 0, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
  g_object_class_install_property(
      object_class, PROP_COOKIE,
      g_param_spec_uint("cookie", "cookie", "cookie", 0, G_MAXINT, 0,
                        G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  dbus_g_object_type_install_info(GSM_TYPE_INHIBITOR,
                                  &dbus_glib_gsm_inhibitor_object_info);
  dbus_g_error_domain_register(GSM_INHIBITOR_ERROR, NULL,
                               GSM_INHIBITOR_TYPE_ERROR);
}

GsmInhibitor *gsm_inhibitor_new(const char *app_id, guint toplevel_xid,
                                guint flags, const char *reason,
                                const char *bus_name, guint cookie) {
  GsmInhibitor *inhibitor;

  inhibitor =
      g_object_new(GSM_TYPE_INHIBITOR, "app-id", app_id, "reason", reason,
                   "bus-name", bus_name, "flags", flags, "toplevel-xid",
                   toplevel_xid, "cookie", cookie, NULL);

  return inhibitor;
}

GsmInhibitor *gsm_inhibitor_new_for_client(const char *client_id,
                                           const char *app_id, guint flags,
                                           const char *reason,
                                           const char *bus_name, guint cookie) {
  GsmInhibitor *inhibitor;

  inhibitor = g_object_new(GSM_TYPE_INHIBITOR, "client-id", client_id, "app-id",
                           app_id, "reason", reason, "bus-name", bus_name,
                           "flags", flags, "cookie", cookie, NULL);
