...
static void gnome_keyring_daemon_finished(GPid pid, gint status,
                                          gpointer user_data) {
  if (WEXITSTATUS(status) != 0) {
    /* daemon failed for some reason */
    g_printerr(
        "gnome-keyring-daemon failed to start correctly, "
        "exit code: %d\n",
        WEXITSTATUS(status));
  }
}

static void gnome_keyring_daemon_startup(void) {
  GError *error = NULL;
  GPid pid;
  gchar *argv[3];
...
static void msm_compat_gnome_smproxy_startup(void) {
  Atom gnome_sm_proxy;
  Display *dpy;
  Window root;
  ...
  if (gnome_smproxy_window != None) XDestroyWindow(dpy, gnome_smproxy_window);

  gnome_sm_proxy = XInternAtom(dpy, "GNOME_SM_PROXY", False);
  gnome_smproxy_window = XCreateSimpleWindow(dpy, root, 1, 1, 1, 1, 0, 0, 0);

  XChangeProperty(dpy, gnome_smproxy_window, gnome_sm_proxy, XA_CARDINAL, 32,
                  PropModeReplace,
                  (unsigned char *)(void *)&gnome_smproxy_window, 1);
  XChangeProperty(dpy, root, gnome_sm_proxy, XA_CARDINAL, 32, PropModeReplace,
                  (unsigned char *)(void *)&gnome_smproxy_window, 1);
...
static void msm_compat_gnome_smproxy_shutdown(void) {
  GdkDisplay *gdkdisplay;
...
void msm_gnome_start(void) {
  ...
  if (gnome_compat_started == TRUE) return;
...
void msm_gnome_stop(void) {
  if (gnome_compat_started == FALSE) return;
