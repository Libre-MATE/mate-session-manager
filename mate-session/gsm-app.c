...
enum { EXITED, DIED, REGISTERED, LAST_SIGNAL };

static guint32 app_serial = 1;

static guint signals[LAST_SIGNAL] = {0};

enum { PROP_0, PROP_ID, PROP_STARTUP_ID, PROP_PHASE, LAST_PROP };
...
static guint32 get_next_app_serial(void) {
  guint32 serial;
...
static gboolean register_app(GsmApp *app) {
  GError *error;
  GsmAppPrivate *priv;
...
static GObject *gsm_app_constructor(
  ...
  app = GSM_APP(
      G_OBJECT_CLASS(gsm_app_parent_class)
          ->constructor(type, n_construct_properties, construct_properties));
  priv = gsm_app_get_instance_private(app);

  g_free(priv->id);
  priv->id =
      g_strdup_printf("/org/gnome/SessionManager/App%u", get_next_app_serial());
...
static void gsm_app_init(GsmApp *app) {}

static void gsm_app_set_phase(GsmApp *app, int phase) {
  GsmAppPrivate *priv;
...
static void gsm_app_set_id(GsmApp *app, const char *id) {
  GsmAppPrivate *priv;
  g_return_if_fail(GSM_IS_APP(app));
...
static void gsm_app_set_startup_id(GsmApp *app, const char *startup_id) {
  GsmAppPrivate *priv;
  g_return_if_fail(GSM_IS_APP(app));
...
static void gsm_app_set_property(GObject *object, guint prop_id,
...
static void gsm_app_get_property(GObject *object, guint prop_id, GValue *value,
                                 GParamSpec *pspec) {
  GsmAppPrivate *priv;
  GsmApp *app = GSM_APP(object);
...
static void gsm_app_dispose(GObject *object) {
  GsmAppPrivate *priv;
  GsmApp *app = GSM_APP(object);
...
static void gsm_app_class_init(GsmAppClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_PHASE,
      g_param_spec_int("phase", "Phase", "Phase", -1, G_MAXINT, -1,
                       G_PARAM_READWRITE));
  g_object_class_install_property(
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("startup-id", "startup ID",
                          "Session management startup ID", NULL,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  signals[EXITED] =
      g_signal_new("exited", G_OBJECT_CLASS_TYPE(object_class),
                   G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GsmAppClass, exited),
                   NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  signals[DIED] =
      g_signal_new("died", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GsmAppClass, died), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[REGISTERED] =
      g_signal_new("registered", G_OBJECT_CLASS_TYPE(object_class),
                   G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GsmAppClass, registered),
                   NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
...
gboolean gsm_app_provides(GsmApp *app, const char *service) {
...
gboolean gsm_app_has_autostart_condition(GsmApp *app, const char *condition) {
...
gboolean gsm_app_start(GsmApp *app, GError **error) {
  GsmAppPrivate *priv;
...
gboolean gsm_app_restart(GsmApp *app, GError **error) {
  GsmAppPrivate *priv;
...
gboolean gsm_app_stop(GsmApp *app, GError **error) {
...
gboolean gsm_app_get_app_id(GsmApp *app, char **id, GError **error) {
  g_return_val_if_fail(GSM_IS_APP(app), FALSE);
  *id = g_strdup(GSM_APP_GET_CLASS(app)->impl_get_app_id(app));
  return TRUE;
}

gboolean gsm_app_get_startup_id(GsmApp *app, char **id, GError **error) {
  GsmAppPrivate *priv;
  g_return_val_if_fail(GSM_IS_APP(app), FALSE);
...
gboolean gsm_app_get_phase(GsmApp *app, guint *phase, GError **error) {
  GsmAppPrivate *priv;
  g_return_val_if_fail(GSM_IS_APP(app), FALSE);
