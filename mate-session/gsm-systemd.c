...
static void gsm_systemd_get_property(GObject *object, guint prop_id,
...
static void gsm_systemd_class_init(GsmSystemdClass *manager_class) {
  ...
  param_spec = g_param_spec_boolean(
      ...
      "Whether the session is connected to Systemd", FALSE, G_PARAM_READABLE);
  ...
  signals[REQUEST_COMPLETED] = g_signal_new(
      ...
      G_STRUCT_OFFSET(GsmSystemdClass, request_completed), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);

  signals[PRIVILEGES_COMPLETED] = g_signal_new(
      "privileges-completed", G_OBJECT_CLASS_TYPE(object_class),
      G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GsmSystemdClass, privileges_completed),
      NULL, NULL, gsm_marshal_VOID__BOOLEAN_BOOLEAN_POINTER, G_TYPE_NONE, 3,
...
static gboolean gsm_systemd_ensure_sd_connection(GsmSystemd *manager,
                                                 GError **error) {
  ...
  if (priv->dbus_connection == NULL) {
    ...
    dbus_connection_add_filter(connection, gsm_systemd_dbus_filter, manager,
                               NULL);
  }
  ...
  if (priv->sd_proxy == NULL) {
    priv->sd_proxy =
        dbus_g_proxy_new_for_name_owner(priv->dbus_connection, SD_NAME, SD_PATH,
                                        SD_INTERFACE, &connection_error);
...
static void gsm_systemd_init(GsmSystemd *manager) {
  GError *error;
...
static void gsm_systemd_free_dbus(GsmSystemd *manager) {
  ...
  if (priv->dbus_connection != NULL) {
    DBusConnection *connection;
    connection = dbus_g_connection_get_connection(priv->dbus_connection);
    dbus_connection_remove_filter(connection, gsm_systemd_dbus_filter, manager);
...
static void emit_restart_complete(GsmSystemd *manager, GError *error) {
  GError *call_error;
  ...
  if (error != NULL) {
    call_error = g_error_new_literal(
        GSM_SYSTEMD_ERROR, GSM_SYSTEMD_ERROR_RESTARTING, error->message);
  }
...
static void emit_stop_complete(GsmSystemd *manager, GError *error) {
  GError *call_error;
  ...
  if (error != NULL) {
    call_error = g_error_new_literal(
        GSM_SYSTEMD_ERROR, GSM_SYSTEMD_ERROR_STOPPING, error->message);
  }
...
gboolean gsm_systemd_is_last_session_for_user(GsmSystemd *manager) {
  ...
  for (i = 0; sessions[i]; i++) {
    ...
    if (g_strcmp0(type, "x11") != 0 && g_strcmp0(type, "wayland") != 0) {
      free(type);
      continue;
    }
    free(type);
  ...
  for (i = 0; sessions[i]; i++) free(sessions[i]);
  free(sessions);
  free(session);
...
void gsm_systemd_attempt_restart(GsmSystemd *manager) {
  ...
  res =
      dbus_g_proxy_call_with_timeout(priv->sd_proxy, "Reboot", INT_MAX, &error,
...
void gsm_systemd_attempt_stop(GsmSystemd *manager) {
  ...
  res = dbus_g_proxy_call_with_timeout(priv->sd_proxy, "PowerOff", INT_MAX,
...
static void gsm_systemd_get_session_path(DBusConnection *connection,
  ...
  message = dbus_message_new_method_call(SD_NAME, SD_PATH, SD_INTERFACE,
                                         "GetSession");
  if (message == NULL) {
    goto out;
  }
...
void gsm_systemd_set_session_idle(GsmSystemd *manager, gboolean is_idle) {
  ...
  gsm_systemd_get_session_path(
      dbus_g_connection_get_connection(priv->dbus_connection), &session_path);

  g_return_if_fail(session_path != NULL);

  g_debug("Updating Systemd idle status: %d", is_idle);
  message = dbus_message_new_method_call(SD_NAME, session_path,
                                         SD_SESSION_INTERFACE, "SetIdleHint");
  if (message == NULL) {
...
gboolean gsm_systemd_can_restart(GsmSystemd *manager) {
  ...
  res = dbus_g_proxy_call_with_timeout(priv->sd_proxy, "CanReboot", INT_MAX,
  ...
  can_restart =
      g_strcmp0(value, "yes") == 0 || g_strcmp0(value, "challenge") == 0;
  g_free(value);
  return can_restart;
}
...
gboolean gsm_systemd_can_stop(GsmSystemd *manager) {
  ...
  res = dbus_g_proxy_call_with_timeout(priv->sd_proxy, "CanPowerOff", INT_MAX,
                                       &error, G_TYPE_INVALID, G_TYPE_STRING,
                                       &value, G_TYPE_INVALID);
  ...
  can_stop = g_strcmp0(value, "yes") == 0 || g_strcmp0(value, "challenge") == 0;
  g_free(value);
  return can_stop;
}
...
gboolean gsm_systemd_can_hibernate(GsmSystemd *manager) {
  ...
  res = dbus_g_proxy_call_with_timeout(priv->sd_proxy, "CanHibernate", INT_MAX,
                                       &error, G_TYPE_INVALID, G_TYPE_STRING,
                                       &value, G_TYPE_INVALID);
  if (res == FALSE) {
    g_warning("Could not make DBUS call: %s", error->message);
...
gboolean gsm_systemd_can_suspend(GsmSystemd *manager) {
  ...
  res = dbus_g_proxy_call_with_timeout(priv->sd_proxy, "CanSuspend", INT_MAX,
                                       &error, G_TYPE_INVALID, G_TYPE_STRING,
                                       &value, G_TYPE_INVALID);
  if (res == FALSE) {
    g_warning("Could not make DBUS call: %s", error->message);
...
void gsm_systemd_attempt_hibernate(GsmSystemd *manager) {
  ...
  res = dbus_g_proxy_call_with_timeout(priv->sd_proxy, "Hibernate", INT_MAX,
                                       &error, G_TYPE_BOOLEAN,
                                       TRUE, /* interactive */
                                       G_TYPE_INVALID, G_TYPE_INVALID);
  if (res == FALSE) {
...
void gsm_systemd_attempt_suspend(GsmSystemd *manager) {
  ...
  res =
      dbus_g_proxy_call_with_timeout(priv->sd_proxy, "Suspend", INT_MAX, &error,
                                     G_TYPE_BOOLEAN, TRUE, /* interactive */
                                     G_TYPE_INVALID, G_TYPE_INVALID);
  if (res == FALSE) {
    g_warning("Could not make DBUS call: %s", error->message);
...
gchar *gsm_systemd_get_current_session_type(GsmSystemd *manager) {
  ...
  if (!gsm_systemd_ensure_sd_connection(manager, &gerror)) {
    g_warning("Could not connect to Systemd: %s", gerror->message);
    g_error_free(gerror);
    return NULL;
  }
