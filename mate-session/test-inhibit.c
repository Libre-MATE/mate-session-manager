...
static gboolean session_manager_connect(void) {
  ...
  if (sm_proxy == NULL) {
    sm_proxy = g_dbus_proxy_new_for_bus_sync(
        G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_NONE, NULL, SM_DBUS_NAME,
        SM_DBUS_PATH, SM_DBUS_INTERFACE, NULL, &error);
  }
...
static gboolean do_inhibit_for_window(GdkWindow *window) {
  ...
  flags = GSM_INHIBITOR_FLAG_LOGOUT | GSM_INHIBITOR_FLAG_SWITCH_USER |
...
static gboolean do_uninhibit(void) {
  GError *error;
  GVariant *ret;

  error = NULL;
  ret = g_dbus_proxy_call_sync(sm_proxy, "Uninhibit",
...
static void on_widget_show(GtkWidget *dialog, gpointer data) {
  gboolean res;
...
int main(int argc, char *argv[]) {
  ...
  dialog =
      gtk_message_dialog_new(NULL, 0, GTK_MESSAGE_INFO, GTK_BUTTONS_CANCEL,
