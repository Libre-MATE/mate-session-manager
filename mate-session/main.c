...
static gboolean initialize_gsettings(void) {
  GSettings* settings;
  time_t now = time(0);
  gboolean ret;
...
static gboolean acquire_name_on_proxy(DBusGProxy* bus_proxy, const char* name) {
  ...
  error = NULL;
  res = dbus_g_proxy_call(bus_proxy, "RequestName", &error, G_TYPE_STRING, name,
  ...
  dbus_g_proxy_connect_signal(bus_proxy, "NameLost",
                              G_CALLBACK(on_bus_name_lost), NULL, NULL);
...
static gboolean acquire_name(void) {
  ...
  if (connection == NULL) {
    gsm_util_init_error(TRUE, "Could not connect to session bus: %s",
                        error->message);
    /* not reached */
  }
...
static void append_default_apps(GsmManager* manager,
  ...
  for (i = 0; default_apps[i]; i++) {
    ...
    app_path = gsm_util_find_desktop_file_for_app_name(default_apps[i],
                                                       autostart_dirs);
...
static void append_required_apps(GsmManager* manager) {
  ...
  required_components =
      g_settings_get_strv(settings, GSM_REQUIRED_COMPONENTS_LIST_KEY);
  ...
  if (required_components == NULL) {
    ...
    for (i = 0; required_components[i]; i++) {
      ...
      default_provider =
          g_settings_get_string(settings_required_components, component);

      g_debug("main: %s looking for component: '%s'", component,
              default_provider);
...
static void maybe_load_saved_session_apps(GsmManager* manager) {
  ...
  if (LOGIND_RUNNING()) {
    ...
    is_login =
        g_strcmp0(session_type, GSM_CONSOLEKIT_SESSION_TYPE_LOGIN_WINDOW) == 0;
#ifdef HAVE_SYSTEMD
  }
#endif
  ...
  if (!is_login) {
    ...
    if (autostart == TRUE)
      gsm_manager_add_autostart_apps_from_dir(manager,
                                              gsm_util_get_saved_session_dir());
  }
...
static void load_override_apps(GsmManager* manager,
                               char** override_autostart_dirs) {
  int i;

  for (i = 0; override_autostart_dirs[i]; i++) {
    gsm_manager_add_autostart_apps_from_dir(manager,
                                            override_autostart_dirs[i]);
  }
}
...
static gboolean schema_exists(const gchar* schema_name) {
...
static gboolean check_gl(gchar** gl_renderer, GError** error) {
  int status;
...
int main(int argc, char** argv) {
  ...
  static GOptionEntry entries[] = {
      {"autostart", 'a', 0, G_OPTION_ARG_STRING_ARRAY, &override_autostart_dirs,
       N_("Override standard autostart directories"), NULL},
      {"debug", 0, 0, G_OPTION_ARG_NONE, &debug, N_("Enable debugging code"),
       NULL},
      ...
      {"disable-acceleration-check", 0, 0, G_OPTION_ARG_NONE,
       &disable_acceleration_check, N_("Disable hardware acceleration check"),
       NULL},
  ...
  error = NULL;
  gtk_init_with_args(&argc, &argv, (char*)_(" - the MATE session manager"),
  ...
  /* Allows to enable/disable debug from GSettings only if it is not set from
   * argument */
  if (!debug && schema_exists(DEBUG_SCHEMA)) {
    debug_settings = g_settings_new(DEBUG_SCHEMA);
    g_signal_connect(debug_settings, "changed::" DEBUG_KEY,
                     G_CALLBACK(debug_changed), NULL);
