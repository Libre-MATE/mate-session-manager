...
enum { PROP_0, PROP_ACTION, PROP_INHIBITOR_STORE, PROP_CLIENT_STORE };
...
static void lock_screen(GsmInhibitDialog *dialog) {
  ...
  if ((screen_locker_command = gsm_get_screen_locker_command()) != NULL) {
    ...
    g_spawn_async(NULL, screen_locker_command, NULL,
                  G_SPAWN_DEFAULT | G_SPAWN_SEARCH_PATH, NULL, NULL, NULL,
                  &error);
...
static void on_response(GsmInhibitDialog *dialog, gint response_id)
...
static void gsm_inhibit_dialog_set_action(GsmInhibitDialog *dialog,
                                          int action) {
  dialog->action = action;
}

static gboolean find_inhibitor(GsmInhibitDialog *dialog, const char *id,
                               GtkTreeIter *iter) {
  GtkTreeModel *model;
  gboolean found_item;
  ...
  do {
    ...
    gtk_tree_model_get(model, iter, INHIBIT_ID_COLUMN, &item_id, -1);
    if (item_id != NULL && id != NULL && strcmp(item_id, id) == 0) {
      found_item = TRUE;
    }
    g_free(item_id);
...
static char *_util_icon_remove_extension(const char *icon) {
...
/* copied from mate-panel panel-util.c */
static char *_find_icon(GtkIconTheme *icon_theme, const char *icon_name,
                        gint size) {
  GtkIconInfo *info;
  char *retval;
  char *icon_no_extension;
  ...
  if (g_path_is_absolute(icon_name)) {
    if (g_file_test(icon_name, G_FILE_TEST_EXISTS)) {
      ...
      retval = _find_icon(icon_theme, basename, size);
      g_free(basename);
...
/* copied from mate-panel panel-util.c */
static GdkPixbuf *_load_icon(GtkIconTheme *icon_theme, const char *icon_name,
                             int size, int desired_width, int desired_height,
                             char **error_msg) {
  GdkPixbuf *retval;
  char *file;
  GError *error;
  ...
  error = NULL;
  retval = gdk_pixbuf_new_from_file_at_size(file, desired_width, desired_height,
                                            &error);
  if (error) {
...
#ifdef HAVE_XRENDER
static GdkPixbuf *pixbuf_get_from_pixmap(Display *display, Pixmap xpixmap,
  ...
  surface = cairo_xlib_surface_create(display, xpixmap, visual, width, height);
  if (surface != NULL) {
...
static Pixmap get_pixmap_for_window(Display *display, Window window,
  ...
  src_picture =
      XRenderCreatePicture(display, window, format, CPSubwindowMode, &pa);

  pixmap = XCreatePixmap(display, window, width, height, attr.depth);

  dst_picture = XRenderCreatePicture(display, pixmap, format, 0, 0);
  XRenderComposite(display, has_alpha ? PictOpOver : PictOpSrc, src_picture,
                   None, dst_picture, 0, 0, 0, 0, 0, 0, width, height);
...
static GdkPixbuf *get_pixbuf_for_window(GdkDisplay *gdkdisplay, guint xid,
...
static void add_inhibitor(GsmInhibitDialog *dialog, GsmInhibitor *inhibitor) {
  ...
  if (desktop_filename != NULL) {
    ...
    if (g_path_is_absolute(desktop_filename)) {
      ...
      if (desktop_file == NULL) {
        if (error) {
          ...
          g_warning("Unable to load desktop file '%s'", desktop_filename);
        }
    ...
    if (desktop_file == NULL) {
      error = NULL;
      desktop_file = egg_desktop_file_new_from_dirs(
          desktop_filename, (const char **)search_dirs, &error);
    }

    /* look for a file with a vendor prefix */
    if (desktop_file == NULL) {
      if (error) {
        g_warning("Unable to find desktop file '%s': %s", desktop_filename,
                  error->message);
        g_error_free(error);
      } else {
  ...
  if (pixbuf == NULL) {
    pixbuf = _load_icon(gtk_icon_theme_get_default(), "mate-windows",
                        DEFAULT_ICON_SIZE, DEFAULT_ICON_SIZE, DEFAULT_ICON_SIZE,
                        NULL);
  }

  gtk_list_store_insert_with_values(
      dialog->list_store, NULL, 0, INHIBIT_IMAGE_COLUMN, pixbuf,
      INHIBIT_NAME_COLUMN, name, INHIBIT_REASON_COLUMN,
      gsm_inhibitor_peek_reason(inhibitor), INHIBIT_ID_COLUMN,
      gsm_inhibitor_peek_id(inhibitor), -1);
...
static gboolean model_has_one_entry(GtkTreeModel *model) {
  guint n_rows;
...
static void update_dialog_text(GsmInhibitDialog *dialog) {
  ...
  if (model_has_one_entry(GTK_TREE_MODEL(dialog->list_store))) {
    ...
    description_text =
        _("Waiting for the program to finish.  Interrupting the program may "
          "cause you to lose work.");
  } else {
    g_debug("Found multiple entries (or none) in model");
    header_text = _("Some programs are still running:");
    description_text =
        _("Waiting for programs to finish.  Interrupting these programs may "
          "cause you to lose work.");
  }

  widget = GTK_WIDGET(gtk_builder_get_object(dialog->xml, "header-label"));
  if (widget != NULL) {
    char *markup;
  ...
  widget = GTK_WIDGET(gtk_builder_get_object(dialog->xml, "description-label"));
  if (widget != NULL) {
...
static void on_store_inhibitor_removed(GsmStore *store, const char *id,
  ...
  if (!gtk_tree_model_get_iter_first(GTK_TREE_MODEL(dialog->list_store),
                                     &iter)) {
    gtk_dialog_response(GTK_DIALOG(dialog), GTK_RESPONSE_ACCEPT);
  }
}

static void gsm_inhibit_dialog_set_inhibitor_store(GsmInhibitDialog *dialog,
                                                   GsmStore *store) {
  ...
  if (dialog->inhibitors != NULL) {
    g_signal_handlers_disconnect_by_func(dialog->inhibitors,
                                         on_store_inhibitor_added, dialog);
    g_signal_handlers_disconnect_by_func(dialog->inhibitors,
                                         on_store_inhibitor_removed, dialog);
  ...
  if (dialog->inhibitors != NULL) {
    g_signal_connect(dialog->inhibitors, "added",
                     G_CALLBACK(on_store_inhibitor_added), dialog);
    g_signal_connect(dialog->inhibitors, "removed",
                     G_CALLBACK(on_store_inhibitor_removed), dialog);
  }
}

static void gsm_inhibit_dialog_set_client_store(GsmInhibitDialog *dialog,
                                                GsmStore *store) {
  g_return_if_fail(GSM_IS_INHIBIT_DIALOG(dialog));
...
static void gsm_inhibit_dialog_set_property(GObject *object, guint prop_id,
                                            const GValue *value,
                                            GParamSpec *pspec) {
...
static void gsm_inhibit_dialog_get_property(GObject *object, guint prop_id,
...
static void name_cell_data_func(GtkTreeViewColumn *tree_column,
                                GtkCellRenderer *cell, GtkTreeModel *model,
                                GtkTreeIter *iter, GsmInhibitDialog *dialog) {
  char *name;
  char *reason;
  char *markup;
  ...
  name = NULL;
  reason = NULL;
  gtk_tree_model_get(model, iter, INHIBIT_NAME_COLUMN, &name,
                     INHIBIT_REASON_COLUMN, &reason, -1);
...
static gboolean add_to_model(const char *id, GsmInhibitor *inhibitor,
                             GsmInhibitDialog *dialog) {
  add_inhibitor(dialog, inhibitor);
  return FALSE;
}

static void populate_model(GsmInhibitDialog *dialog) {
  gsm_store_foreach_remove(dialog->inhibitors, (GsmStoreFunc)add_to_model,
                           dialog);
  update_dialog_text(dialog);
}

static void setup_dialog(GsmInhibitDialog *dialog) {
  ...
  gtk_dialog_add_button(GTK_DIALOG(dialog), _("Lock Screen"),
  ...
  dialog->list_store =
      gtk_list_store_new(NUMBER_OF_COLUMNS, GDK_TYPE_PIXBUF, G_TYPE_STRING,
                         G_TYPE_STRING, G_TYPE_STRING);

  treeview =
      GTK_WIDGET(gtk_builder_get_object(dialog->xml, "inhibitors-treeview"));
  ...
  gtk_tree_view_column_set_cell_data_func(
      column, renderer, (GtkTreeCellDataFunc)name_cell_data_func, dialog, NULL);
...
static GObject *gsm_inhibit_dialog_constructor(
  ...
  GdkDisplay *gdkdisplay;
#endif /* HAVE_XRENDER */
  dialog = GSM_INHIBIT_DIALOG(
      G_OBJECT_CLASS(gsm_inhibit_dialog_parent_class)
          ->constructor(type, n_construct_properties, construct_properties));

#ifdef HAVE_XRENDER
  gdkdisplay = gdk_display_get_default();
  gdk_x11_display_error_trap_push(gdkdisplay);
  if (XRenderQueryExtension(GDK_DISPLAY_XDISPLAY(gdkdisplay),
                            &dialog->xrender_event_base,
                            &dialog->xrender_error_base)) {
...
static void gsm_inhibit_dialog_dispose(GObject *object) {
  GsmInhibitDialog *dialog;
...
static void gsm_inhibit_dialog_class_init(GsmInhibitDialogClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_ACTION,
      g_param_spec_int("action", "action", "action", -1, G_MAXINT, -1,
                       G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
  g_object_class_install_property(
      object_class, PROP_INHIBITOR_STORE,
      g_param_spec_object("inhibitor-store", NULL, NULL, GSM_TYPE_STORE,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
  g_object_class_install_property(
...
static void gsm_inhibit_dialog_init(GsmInhibitDialog *dialog) {
  ...
  widget = GTK_WIDGET(gtk_builder_get_object(dialog->xml, "main-box"));
...
GtkWidget *gsm_inhibit_dialog_new(GsmStore *inhibitors, GsmStore *clients,
  ...
  object =
      g_object_new(GSM_TYPE_INHIBIT_DIALOG, "action", action, "inhibitor-store",
                   inhibitors, "client-store", clients, NULL);
