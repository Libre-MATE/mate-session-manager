...
enum { PROP_0, PROP_MESSAGE_TYPE };

G_DEFINE_TYPE(GsmLogoutDialog, gsm_logout_dialog, GTK_TYPE_MESSAGE_DIALOG);

static void gsm_logout_dialog_set_property(GObject *object, guint prop_id,
                                           const GValue *value,
                                           GParamSpec *pspec) {
  switch (prop_id) {
...
static void gsm_logout_dialog_get_property(GObject *object, guint prop_id,
...
static void gsm_logout_dialog_class_init(GsmLogoutDialogClass *klass) {
  ...
  g_object_class_override_property(gobject_class, PROP_MESSAGE_TYPE,
                                   "message-type");
}

static void gsm_logout_dialog_init(GsmLogoutDialog *logout_dialog) {
  ...
  g_signal_connect(logout_dialog, "destroy",
                   G_CALLBACK(gsm_logout_dialog_destroy), NULL);

  g_signal_connect(logout_dialog, "show", G_CALLBACK(gsm_logout_dialog_show),
                   NULL);
}
...
static gboolean gsm_logout_supports_switch_user(
    GsmLogoutDialog *logout_dialog) {
  GSettings *settings;
  gboolean ret = FALSE;
  gboolean locked;
...
static gboolean gsm_logout_supports_reboot(GsmLogoutDialog *logout_dialog) {
  gboolean ret;
...
static gboolean gsm_logout_supports_shutdown(GsmLogoutDialog *logout_dialog) {
  gboolean ret;
...
static gboolean gsm_logout_dialog_timeout(gpointer data) {
  GsmLogoutDialog *logout_dialog;
  ...
  if (session_type == NULL) {
    ...
    if (LOGIND_RUNNING()) {
      ...
      is_not_login =
          (g_strcmp0(session_type, GSM_SYSTEMD_SESSION_TYPE_LOGIN_WINDOW) != 0);
    } else {
#endif
      GsmConsolekit *consolekit;
      ...
      is_not_login = (g_strcmp0(session_type,
                                GSM_CONSOLEKIT_SESSION_TYPE_LOGIN_WINDOW) != 0);
#ifdef HAVE_SYSTEMD
    }
#endif
  }
  ...
  if (is_not_login) {
    ...
    secondary_text =
        g_strdup_printf(_("You are currently logged in as \"%s\"."), name);
...
static void gsm_logout_dialog_set_timeout(GsmLogoutDialog *logout_dialog) {
  ...
  if (logout_dialog->timeout > 0) {
    ...
    logout_dialog->timeout_id =
        g_timeout_add(1000, gsm_logout_dialog_timeout, logout_dialog);
  } else {
...
static GtkWidget *gsm_get_dialog(GsmDialogLogoutType type, GdkScreen *screen,
  ...
  switch (type) {
    case GSM_DIALOG_LOGOUT_TYPE_LOGOUT:
      ...
      if (gsm_logout_supports_switch_user(logout_dialog)) {
        gsm_util_dialog_add_button(GTK_DIALOG(logout_dialog), _("_Switch User"),
                                   "system-users",
                                   GSM_LOGOUT_RESPONSE_SWITCH_USER);
      }

      gsm_util_dialog_add_button(GTK_DIALOG(logout_dialog), _("_Cancel"),
                                 "process-stop", GTK_RESPONSE_CANCEL);

      gsm_util_dialog_add_button(GTK_DIALOG(logout_dialog), _("_Log Out"),
                                 "system-log-out", GSM_LOGOUT_RESPONSE_LOGOUT);
    ...
    case GSM_DIALOG_LOGOUT_TYPE_SHUTDOWN:
      ...
      if (gsm_logout_supports_system_suspend(logout_dialog)) {
        gsm_util_dialog_add_button(GTK_DIALOG(logout_dialog), _("S_uspend"),
                                   "battery", GSM_LOGOUT_RESPONSE_SLEEP);
      }

      if (gsm_logout_supports_system_hibernate(logout_dialog)) {
        gsm_util_dialog_add_button(GTK_DIALOG(logout_dialog), _("_Hibernate"),
                                   "drive-harddisk",
                                   GSM_LOGOUT_RESPONSE_HIBERNATE);
      }

      if (gsm_logout_supports_reboot(logout_dialog)) {
        gsm_util_dialog_add_button(GTK_DIALOG(logout_dialog), _("_Restart"),
                                   "view-refresh", GSM_LOGOUT_RESPONSE_REBOOT);
      }
      ...
      if (gsm_logout_supports_shutdown(logout_dialog)) {
        gsm_util_dialog_add_button(GTK_DIALOG(logout_dialog), _("_Shut Down"),
                                   "system-shutdown",
  ...
  gtk_progress_bar_set_show_text(GTK_PROGRESS_BAR(logout_dialog->progressbar),
                                 TRUE);
  gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(logout_dialog->progressbar),
                                1.0);
  gtk_box_pack_start(GTK_BOX(hbox), logout_dialog->progressbar, TRUE, TRUE, 12);
  gtk_widget_show_all(hbox);
  gtk_container_add(
      GTK_CONTAINER(gtk_dialog_get_content_area(GTK_DIALOG(logout_dialog))),
      hbox);
...
GtkWidget *gsm_get_shutdown_dialog(GdkScreen *screen, guint32 activate_time) {
  return gsm_get_dialog(GSM_DIALOG_LOGOUT_TYPE_SHUTDOWN, screen, activate_time);
}

GtkWidget *gsm_get_logout_dialog(GdkScreen *screen, guint32 activate_time) {
  return gsm_get_dialog(GSM_DIALOG_LOGOUT_TYPE_LOGOUT, screen, activate_time);
}
