...
void gsm_store_foreach(GsmStore *store, GsmStoreFunc func, gpointer user_data) {
  ...
  g_hash_table_find(priv->objects, (GHRFunc)func, user_data);
}
...
GObject *gsm_store_find(GsmStore *store, GsmStoreFunc predicate,
  ...
  object = g_hash_table_find(priv->objects, (GHRFunc)predicate, user_data);
  return object;
}
...
static gboolean foreach_remove_wrapper(const char *id, GObject *object,
                                       WrapperData *data) {
  gboolean res;
...
guint gsm_store_foreach_remove(GsmStore *store, GsmStoreFunc func,
  ...
  ret = g_hash_table_foreach_remove(priv->objects,
                                    (GHRFunc)foreach_remove_wrapper, &data);
...
static gboolean _remove_all(const char *id, GObject *object, gpointer data) {
  return TRUE;
}
...
void gsm_store_clear(GsmStore *store) {
  ...
  gsm_store_foreach_remove(store, _remove_all, NULL);
}
...
static void gsm_store_set_property(GObject *object, guint prop_id,
...
static void gsm_store_get_property(GObject *object, guint prop_id,
...
static void gsm_store_dispose(GObject *object) {
  GsmStore *store;
...
static void gsm_store_class_init(GsmStoreClass *klass) {
  ...
  signals[ADDED] = g_signal_new(
      "added", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GsmStoreClass, added), NULL, NULL,
      g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);
  signals[REMOVED] = g_signal_new(
      "removed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GsmStoreClass, removed), NULL, NULL,
      g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);
  g_object_class_install_property(
      object_class, PROP_LOCKED,
      g_param_spec_boolean("locked", NULL, NULL, FALSE,
                           G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}

static void _destroy_object(GObject *object) {
...
static void gsm_store_init(GsmStore *store) {
  ...
  priv->objects = g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
...
GsmStore *gsm_store_new(void) {
  ...
  object = g_object_new(GSM_TYPE_STORE, NULL);
